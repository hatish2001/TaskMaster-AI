...
import { RepoIndex, similaritySearch } from "./indexer";
...
export async function runPlanningPipeline(
  ...
  const topChunks = similaritySearch(repoIndex, queryEmbedding, 12);
  ...
  const pmMessages = [
    new SystemMessage(`You are product_manager_agent. Build a comprehensive product spec grounded in the provided repository context.
...
- Incorporate explicit metrics, timeline ranges, and team sizing when possible.`),
    new HumanMessage(`Initial Goal: ${goal}
...
${context}`),
  ...
  const uxMessages = [
    new SystemMessage(`You are ux_designer_agent. Create a detailed UX implementation brief grounded in the product spec and repo context.
Return strict JSON:
{
...
function renderTicket(
  ...
  const refs = (pmData?.repo_refs ?? []) as RepoRef[];
  ...
  const objectives = Array.isArray(pmData?.objectives)
    ? pmData.objectives.map((objective: any) => {
  ...
  const functionalRequirements = Array.isArray(
    pmData?.functional_requirements
  )
    ? (pmData.functional_requirements as FunctionalRequirement[]).map((req) => {
        const title = req.title ?? "TBD Requirement";
        const priority = (req as any)?.priority
          ? ` (${(req as any).priority})`
          : "";
  ...
  const nonFunctionalRequirements = Array.isArray(
    pmData?.non_functional_requirements
  )
    ? pmData.non_functional_requirements.map((item: any) => {
  ...
  const migrationPhases = Array.isArray(migrationStrategy?.phased_plan)
    ? migrationStrategy.phased_plan.map((phase: any) => {
        const name = phase?.phase ?? "Phase TBD";
        const goals = formatList(phase?.goals);
        const parallel =
          typeof phase?.parallel_streamlit === "boolean"
  ...
  const apiContracts = Array.isArray(pmData?.api_contracts)
    ? pmData.api_contracts.map((api: any) => {
  ...
  const costBenefitSection = [
    `- Development Effort: ${
      costBenefit?.development_effort ?? "TBD"
    }`,
  ...
  const deliveryPlan = Array.isArray(pmData?.delivery_plan)
    ? (pmData.delivery_plan as DeliveryPlan[]).map((phase) => {
        const name = phase.phase ?? "Phase TBD";
        const owner = phase.owner ? ` (Owner: ${phase.owner})` : "";
        const duration =
          (phase as any)?.duration ?? "Duration TBD";
        const team = (phase as any)?.team
          ? ` (Team: ${(phase as any).team})`
          : "";
  ...
  const userFlows = Array.isArray(uxData?.user_flows)
    ? (uxData.user_flows as UserFlow[]).map((flow) => {
  ...
  const communicationPlan = Array.isArray(pmData?.communication_plan)
    ? pmData.communication_plan.map((entry: any) => {
  ...
  const decisionLog = Array.isArray(pmData?.decision_log)
    ? pmData.decision_log.map((item: any) => {
...
function indentList(value: string): string {
  if (!value) {
    return "  - TBD";
  }
  return value
...
function buildFallbackRepoSection(
  paths: Set<string>,
...
function summarizeChunksByPath(chunks: RepoChunk[]): Map<string, string[]> {
...
function summarizeChunk(chunk: RepoChunk): string | undefined {
  if (!chunk.content) {
    return undefined;
  }
  ...
  const candidate =
    lines.find((line) => !isCommentLine(line)) ?? lines[0] ?? undefined;
  if (!candidate) {
    return undefined;
  }
